...
return (<div>
        <Well>
        You currently have access to the Quick Size calculator, which is free of charge. <p></p> 
        Upgrade to Quick Fit for the following additional features:<p></p>
          <ul>
...
return (
    ...
    <Button id="changeSubscription" value="changeSubscription" onClick={props.setSubscriptionView}>Change Your Subscription</Button>
    </div>)
...
case 'trialing and cancelation pending':
  return(
    ...
    <Button id="reactivateSubscription" value="reactivateSubscription" onClick={props.toggleReactivateSubscription}>Reactivate Your Subscription</Button>
    </div>
  )
...
case 'active':
  return(
    <div>
    <Well><p>You are currently subscribed to: <b>Quick Fit</b></p>
    <p>Your payment frequency is: <b>
    {props.stripeSubscription.plan.interval==="month"&&" Monthly"}
    {props.stripeSubscription.plan.interval==="year"&&" Yearly"}</b>
    </p>
    <p>Your subscription will automatically renew on: <b>{props.currentStop.toDateString()}</b>
    </p>
    <PaymentMethod 
      user={props.user}
      stripeCard={props.stripeCard}
  ...
  case 'active and cancelation pending':
    ...
    <div>
    <Well>
    <p><Glyphicon glyph="exclamation-sign"/> Cancelation Pending.</p>  
    <p>You are currently subscribed to: <b>Quick Fit</b></p>
    <p>However, You previously canceled your subscription.</p>
    <p>Your subscription will be canceled automatically at the end of the current billing period on: <b>{props.currentStop.toDateString()}</b></p>
    <p>You may reactivate your subscription at any time prior to cancelation.</p>   
    </Well> 
    <Button onClick={props.toggleReactivateSubscription}>Reactivate Your Subscription</Button>
    </div>  
    )
  ...
  case 'canceled':
    return(
      <div>
      <Well>
      <p>Your subscription to Quick Fit was canceled on <b>{props.canceledAt.toDateString()}</b> and is no longer active.</p>
      <p>But don't worry, all of your data is saved.</p>
      <p>To use Quick Fit again and restore access to your data, please purchase a new subscription.</p>
      </Well>
      <Button id="existingCustomer" value="existingCustomer" onClick={props.setSubscriptionView}>Purchase New Subscription</Button>
      </div>
    )
