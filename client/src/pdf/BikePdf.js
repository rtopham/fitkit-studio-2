...
const bikePDF = (
  ...
  const textMidPoint = 0.5 * gridSize + 0.5 * lineHeight
  ...
  let bikeString = '',
    saddleString = ''
  if (!bike.make && !bike.model) bikeString = 'Unspecified Bike'
  else bikeString = bike.make + ' ' + bike.model
  ...
  pdf.textWithLink(user.email, FTlabelColumn + 11, line[4], {
  ...
  pdf.setTextColor(11, 0, 128)
  pdf.textWithLink(shop.website, STlabelColumn, line[shopArray.length], {
    url: shop.website,
  })
  ...
  pdf.text(
    CPdataColumn,
    line[2],
    cyclistProfile.firstName + ' ' + cyclistProfile.lastName
  )
  pdf.text(CPlabelColumn, line[3], 'Email: ')
  pdf.setTextColor(11, 0, 128)
  pdf.textWithLink(cyclistProfile.email, CPdataColumn, line[3], {
    url: 'mailto:' + cyclistProfile.email,
  })
  pdf.setTextColor(0, 0, 0)
  ...
  */
  ...
  if (bike.type !== 'Mountain Bike' && bike.type !== 'TT/Tri Bike') {
    ...
    pdf.text(
      FPlabelColumn,
      line[3],
      'Saddle Height (saddle to bottom bracket):'
    )
...
*/
  ...
  if (bike.type !== 'Mountain Bike' && bike.type !== 'TT/Tri Bike')
    NfirstLine = NfirstLine + lineHeight * 3
  line[0] = NfirstLine
  for (i = 1; i < 20; i++) {
    line[i] = line[i - 1] + lineHeight + lineMargin
  }
...
*/
  ...
  const fileDate = new Date(updated).toDateString()
  const fileDateString = fileDate.replace(/ /g, '-')

  pdf.save(
    'FKS_Fit_Report_' +
      cyclistProfile.firstName +
      '_' +
      cyclistProfile.lastName +
      '_' +
      bikeString.replace(' ', '_') +
      '_' +
      fileDateString +
      '.pdf'
  )
