...
class EditAccount extends Component {
  ...
  render() {
    ...
    let serviceLevel = ''
    if (this.state.stripeSubscription.plan.interval === 'month')
      serviceLevel = 'Quick Fit (Monthly)'
    if (this.state.stripeSubscription.plan.interval === 'year')
      serviceLevel = 'Quick Fit (Yearly)'
    if (this.state.stripeSubscription.plan.interval === undefined)
      serviceLevel = 'Quick Size'

    return (
      <div className="globalCore">
        <div>
          <ListGroup>
            <ListGroupItem header={this.state.originalUser.name}>
              {this.state.originalUser.email}
            </ListGroupItem>
            <ListGroupItem>
              {'Joined: ' +
                new Date(this.state.originalUser.created).toDateString()}
            </ListGroupItem>
            <ListGroupItem>
              {'Current Service Level: ' + serviceLevel}{' '}
            </ListGroupItem>
          </ListGroup>
        </div>
        <EditProfile
        ...
        />
        <EditPreferences
          changeHeightUnits={this.changeHeightUnits}
          changeWeightUnits={this.changeWeightUnits}
          user={this.state.user}
        />
        {this.state.user.shop_owner && (
          <EditShopStudio
            unsavedShopChanges={this.state.unsavedShopChanges}
            updateLogoState={this.updateLogoState}
            changeShopStatus={this.changeShopStatus}
            changeShopStudio={this.changeShopStudio}
            tempLogo={this.state.tempLogo}
            logoUrl={this.state.logoUrl}
            shop={this.state.shop}
            removeDeletedShopFromState={this.removeDeletedShopFromState}
            user={this.state.user}
          />
        )}
        {!this.state.user.shop_owner && (
        ...
        {this.state.user.admin && (
          <Link to={'/admin/' + this.state.user._id}>Access Admin Page</Link>
        )}
      </div>
    )
  }
